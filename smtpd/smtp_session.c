...
static void
header_domain_append_callback(struct smtp_session *s, const char *hdr,
    const char *val)
{
	size_t			i, j, linelen;
	...
	if (smtp_message_printf(s, "%s:", hdr) == -1)
		return;
	...
	for (line = val; line; line = end) {
		...
		for (i = 0; i < linelen; ++i) {
			...
			if (line[i] == ',' && !escape && !quote && !comment) {
				...
				if (skip) {
					if (smtp_message_printf(s, "%c", line[i]) == -1)
						return;
				}
				else {
					buffer[j++] = line[i];
	...
	for(;;) {
		...
		case RFC5322_MSG_HDR_END:
			if (!strcasecmp("To", res.hdr) ||
			    !strcasecmp("Cc", res.hdr) ||
			    !strcasecmp("From", res.hdr))
				header_domain_append_callback(s, res.hdr,
				    res.value);
			break;
		...
		case RFC5322_MSG_HDRS_END:
			if (s->listener->local ||
				...
				if (!tx->has_date) {
					log_debug("debug: %p: adding Date", s);
					smtp_message_printf(s, "Date: %s\n",
					    time_to_text(tx->time));
				}

				if (!tx->has_message_id) {
					log_debug("debug: %p: adding Message-ID", s);
					smtp_message_printf(s,
